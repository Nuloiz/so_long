...
static void	exit_program(t_vars *vars, int keycode)
{
	if (vars->num_c > vars->current_c && keycode != 65307)
		return ;
...
static int	handle_event(t_vars *vars)
{
	mlx_destroy_image(vars->mlx, vars->textures->cd);
	mlx_destroy_image(vars->mlx, vars->textures->csa);
	mlx_destroy_image(vars->mlx, vars->textures->csc);
...
void	moves(t_vars *vars)
{
	...
	mlx_hook(vars->win, 17, 0L, handle_event, vars);
